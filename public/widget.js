...
var hist=[],isOpen=false,leadCaptured=false;

function saveConv(extra){
  var p={sessionId:SESSION_ID,messages:hist,pageUrl:window.location.href};
...
+'#pc-lead-row input{flex:1;border:1.5px solid #e8e0f0;border-radius:20px;padding:7px 12px;font-size:12.5px;outline:none;font-family:inherit;min-width:0;}'
...
bubble.id='pc-bubble';
bubble.innerHTML='<div id="pc-dot"></div><span style="font-size:28px">🐾</span>';
...
+'<div id="pc-book-bar"><span style="font-size:12px;color:#666;font-weight:500">📅 Book a grooming session</span><button id="pc-book-btn">📆 Book a Grooming</button></div>'
+'<div id="pc-msgs"></div>'
...
+'<div id="pc-lead-section" style="display:none"><div id="pc-lead-title">Share your contact for faster support (optional)</div><div id="pc-lead-row"><input id="pc-lead-name" type="text" placeholder="Your name"/><input id="pc-lead-phone" type="tel" placeholder="Phone"/><button id="pc-lead-submit">Save</button></div></div>'
...
var msgs=document.getElementById('pc-msgs'),inp=document.getElementById('pc-input'),dot=document.getElementById('pc-dot'),promptsEl=document.getElementById('pc-prompts');

function ts(){var d=new Date();return d.getHours()+':'+String(d.getMinutes()).padStart(2,'0');}
function addMsg(t,r,nm){var el=document.createElement('div');el.className='pc-m '+r;el.textContent=t;msgs.appendChild(el);if(!nm){var m=document.createElement('div');m.className='pc-meta'+(r==='user'?' um':'');m.textContent=ts();msgs.appendChild(m);}msgs.scrollTop=msgs.scrollHeight;}
...
function showPrompts(){
  promptsEl.innerHTML='';
  PROMPTS.forEach(function(p){var btn=document.createElement('button');btn.className='pc-prompt';btn.textContent=p;btn.onclick=function(){promptsEl.style.display='none';sendMsg(p);};promptsEl.appendChild(btn);});
...
function sendMsg(t){
  ...
  showTyping();
  if(hist.length===3&&!leadCaptured){var ls=document.getElementById('pc-lead-section');if(ls)ls.style.display='block';}
  saveConv();
  fetch(CHAT_URL+'/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:hist})})
    .then(function(r){return r.json();})
    .then(function(d){hideTyping();addMsg(d.reply,'bot');hist.push({role:'assistant',content:d.reply});saveConv();if(hist.length<8)showPrompts();})
...
document.getElementById('pc-book-btn').addEventListener('click',function(){window.open(BOOKING_URL,'_blank');addSys('Opening grooming booking page for you!');saveConv({event:'booking_clicked'});});
document.getElementById('pc-human-btn').addEventListener('click',function(){addSys('Connecting you with a human agent via WhatsApp...');saveConv({event:'human_handover'});setTimeout(function(){window.open(WHATSAPP_URL,'_blank');},600);});
...
document.getElementById('pc-lead-submit').addEventListener('click',function(){
  var nm=document.getElementById('pc-lead-name').value.trim();
  var ph=document.getElementById('pc-lead-phone').value.trim();
  if(nm||ph){leadCaptured=true;document.getElementById('pc-lead-section').style.display='none';addSys('Thanks '+(nm||'')+'! Our team may reach out if needed.');saveConv({lead:{name:nm,phone:ph}});}
});
